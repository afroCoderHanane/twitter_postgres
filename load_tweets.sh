...
for file in $files; do
    ...
    time python3 -u ./load_tweets.py --db="postgresql://postgres:pass@localhost:8728" --inputs=$file
done

echo 'load denormalized'
for file in $files; do
    # use SQL's COPY command to load data into pg_denormalized
    time unzip -p $file | sed 's/\\u0000//g' | psql postgres://postgres:pass@localhost:3037/ -c "COPY tweets_jsonb (data) FROM STDIN csv quote e'\x01' delimiter e'\x02';"
done
